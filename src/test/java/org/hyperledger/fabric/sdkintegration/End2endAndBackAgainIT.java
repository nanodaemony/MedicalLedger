...
public class End2endAndBackAgainIT {
    ...
    private static boolean checkInstalledChaincode(HFClient client, Peer peer, String ccName, String ccPath, String ccVersion) throws InvalidArgumentException, ProposalException {
        print("Checking installed chaincode: %s, at version: %s, on peer: %s", ccName, ccVersion, peer.getName());
    ...
    public void setupUsers(SampleStore sampleStore) {
        ...
        // 获取全部组织并进行操作
        for (SampleOrg organization : testSampleOrgs) {
            ...
            MedicalUser user = sampleStore.getMember(TESTUSER_1_NAME, orgName);
            // Remember user belongs to this Org
            organization.addUser(user);

            organization.setAdminPeer(sampleStore.getMember(orgName + "Admin", orgName));
        }
    }
...
//            final boolean changeContext = false; // BAR_CHANNEL_NAME.equals(channel.getName()) ? true : false;
            ...
            print("Running Channel %s with a delta %d", channelName, delta);

            print("ChaincodeID: ", chaincodeID);
            ////////////////////////////
            ...
            moveAmount(client, channel, chaincodeID, "25", changeContext ? sampleOrg.getAdminPeer() : null).thenApply((BlockEvent.TransactionEvent transactionEvent) -> {
                try {
                    ...
                    client.setUserContext(sampleOrg.getAdminPeer());
                    ///////////////
                    ////
                    ...
                    if (changeContext) {
                        installProposalRequest.setUserContext(sampleOrg.getAdminPeer());
                    }

                    print("Sending install proposal for channel: %s", channel.getName());
                    ...
                    print("Received %d install proposal responses. Successful+verified: %d . Failed: %d", numInstallProposal, successful.size(), failed.size());
                    ...
                    if (changeContext) {
                        upgradeProposalRequest.setUserContext(sampleOrg.getAdminPeer());
                    }

                    print("Sending upgrade proposal");
                    ...
                    for (ProposalResponse response : responses2) {
                        if (response.getStatus() == Status.SUCCESS) {
                            print("Successful upgrade proposal response Txid: %s from peer %s", response.getTransactionID(), response.getPeer().getName());
                            successful.add(response);
                        } else {
                            failed.add(response);
                        }
                    ...
                    print("Received %d upgrade proposal responses. Successful+verified: %d . Failed: %d", channel.getPeers().size(), successful.size(), failed.size());
                    ...
                    if (changeContext) {
                        return channel.sendTransaction(successful, sampleOrg.getAdminPeer()).get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS);
                ...
                try {
                    ...
                    print("Chaincode has been upgraded to version %s", CHAIN_CODE_VERSION_11);
                    ...
                    return moveAmount(client, channel, chaincodeID_11, "50",
                            changeContext ? sampleOrg.getAdminPeer() : null).get(testConfig.getTransactionWaitTime(), TimeUnit.SECONDS); // really move 100
                } catch (CompletionException e) {
                    throw e;
                } catch (Exception e) {
                    throw new CompletionException(e);
    ...
    CompletableFuture<BlockEvent.TransactionEvent> moveAmount(HFClient client, Channel channel, ChaincodeID chaincodeID, String moveAmount, User user) {
        ...
        try {
            ...
            print("sending transaction proposal to all peers with arguments: move(a,b,%s)", moveAmount);
            ...
            print("Sending chaincode transaction(move a,b,%s) to orderer.", moveAmount);
            if (user != null) {
    ...
    private Channel reconstructChannel(String name, HFClient client, SampleOrg sampleOrg) throws Exception {
        print("Reconstructing %s channel", name);
        ...
        if (BAR_CHANNEL_NAME.equals(name)) { // bar channel was stored in samplestore in End2endIT testcase.
            ...
            print("Retrieved channel %s from sample store.", name);
        ...
        print("Finished reconstructing channel %s.", name);
...
//                out("blockwalker %b, start: %d, stop: %d, i: %d, block %d", useFilteredBlocks, start, stopValue.longValue(), i, blockEvent.getBlockNumber());
    ...
    private void queryChaincodeForExpectedValue(HFClient client, Channel channel, final String expect, ChaincodeID chaincodeID) {
        ...
        print("Now query chaincode %s on channel %s for the value of b expecting to see: %s", chaincodeID, channel.getName(), expect);
