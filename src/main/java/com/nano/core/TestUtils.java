...
public class TestUtils {
     ...
     * @param channel 通道
     */
    public static void testRemovingAddingPeersOrderers(HFClient fabricClient, Channel channel) {
        // Peer相关的Map
        Map<Peer, Channel.PeerOptions> peerOptionsMap = new HashMap<>();
        // 确保传入的channel是初始化且运行着的
        assertTrue(channel.isInitialized());
        assertFalse(channel.isShutdown());

        try {
            // time needed let channel get config block
            // 等待15秒让channel获取配置区块
            Thread.sleep(1500);
        ...
        channel.getPeers().forEach(peer -> {
            try {
                peerOptionsMap.put(peer, channel.getPeersOptions(peer));
        ...
        peerOptionsMap.forEach((peer, value) -> {
            try {
                // 构造新的Peer结点
                Peer newPeer = fabricClient.newPeer(peer.getName(), peer.getUrl(), peer.getProperties());
                // 将新的Peer加入Channel中
        ...
        // 将list里面的Order结点再次加入到Channel中
        removedOrders.forEach(orderer -> {
            try {
                Orderer newOrderer = fabricClient.newOrderer(orderer.getName(), orderer.getUrl(), orderer.getProperties());
